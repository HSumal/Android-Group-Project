...
/**
 * A class to represent a FlightDatabase. An object that will store all the Flights from file, or by
 * hand from Users.
 * 
...
public class FlightDatabase implements Serializable {
  ...
  private static final long serialVersionUID = 3813321155091484198L;
  // Instance Variables
  ...
  /**
   * returns a List of flights formatted how they would be found in a file.
   * @return a List of flights formatted how they would be found in a file
     */
  public List<String> getAllFlightsString (){
    List<String> allFlights = new ArrayList<>();
    Set<Flight> flights = database.keySet();
    for (Flight flight : flights) {
      String newFlightString = flight.toString() + ";" + flight.getNumSeats();
      allFlights.add(newFlightString);
    }
    return allFlights;
  }
  ...
  public void addFlightFile(String filename)
    ...
    while ((line = reader.readLine()) != null) {
      ...
      Flight flightToAdd = new Flight(lineList.get(0), lineList.get(1), lineList.get(2),
          ...
          Integer.parseInt(lineList.get(7)));
      ...
      if (flight.getOrigin().equals(origin)) {
        ...
        if (flight.getDeparture().equals(departure)) {
          ...
          if (flight.getDestination().equals(destination)) {
            ...
            // destination
            // itinerariesSet.addAll(makeItineraries(destination,
            // flight));
          }
        }
      }
  ...
  /*
   * // recursive helper private Set<ArrayList<Flight>> makeItineraries(String destination, Flight
   * flight) { int secondToMilliSecond = 1000; Set<ArrayList<Flight>> itineraries = new
   * HashSet<ArrayList<Flight>>(); // finds flights which links to flight Set<Flight>
   * neighbourFlights = database.get(flight); for (Flight neighbourFlight : neighbourFlights) { //
   * ensures that the time flight arriving and neighbourFlight // departing is between the min //
   * and max layover // time between neighbourFlight leaving and flight arriving long timeBetween =
   * neighbourFlight.getDeparture().getTime() - flight.getArrival().getTime();
   * 
   * if (timeBetween <= (Driver.MAX_LAYOVER.getSeconds() * secondToMilliSecond) && timeBetween >=
   * (Driver.MIN_LAYOVER.getSeconds() * secondToMilliSecond)) {
   * 
   * // base case, if Flight with desired destination is found if
   * (neighbourFlight.getDestination().equals(destination)) { ArrayList<Flight> itinerary = new
   * ArrayList<>(); itinerary.add(neighbourFlight); itinerary.add(0, flight);
   * itineraries.add(itinerary); } else { // occur if neighbourFlight doesn't land at destination //
   * recursively checks if flight links to any flights to // destination Set<ArrayList<Flight>>
   * itinerariesHelper = makeItineraries(destination, neighbourFlight); for (ArrayList<Flight>
   * itinerary : itinerariesHelper) { itinerary.add(0, flight); itineraries.add(itinerary); } } } }
   * 
   * return itineraries;
   * 
   * }
